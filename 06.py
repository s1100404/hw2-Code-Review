...
class character:                                                  #自訂類別 character
    def __init__(self, name, life, x, y, imgfile, tagname):       # warrior 和 magician的相同程式碼(8~41)
        self.name = name
        self.life = life
        self.lmax = life
        self.x = x
...
class Warrior(character):                                                #warrior(繼承)
    def __init__(self, name, life, x, y, imgfile, tagname):              #帶入相同類別
...
class Magician(character):                                                #magician(繼承)
    def __init__(self, name, life, x, y, imgfile, tagname):               #帶入相同類別
        super().__init__(name, life, x, y, imgfile, tagname)
...
def click_left():                                                #左按鍵 warrior攻擊/magician受傷害
    character[0].attack()
    character[1].damage()


def click_right():                                               #右按鍵 magician攻擊/warrior受傷害
    character[0].attack()
    character[1].attack()
    character[0].damage()
