...
FNT=("Times New Romen",20)


class profession:
    def __init__(self, name,classname, life, x, y, imgfile, tagname,attack_mode):
        self.name=name
        ...
        self.attack_made= attack_mode
     
    def draw(self):
        x=self.x
        y=self.y
        canvas.create_image(x,y,image=self.img,tag=self.tagname)
        canvas.create_taxt(x,y+200,text=self.name,font=FNT,fill='red', 
                           tag=self.tagname)
        canvas.create_taxt(x,y+230,text=self.name,font=FNT,fill='bule', 
                           tag=self.tagname)
        canvas.create_taxt(x,y+260,text=f"HP:{self.life}/{self.lmax}",
...
class Warrior(profession):
    def __init__(self,name,life,x,y,imgfile,taname):
        super().__init__(name,"Worrior",life,x,y,imgfile,taname,"Sword Attack")
class Magician(profession):
    def __init__(self,name,classname,life,x,y,imgfile,taname,attack_mode):
        super().__init__(name,"Magician",life,x,y,imgfile,taname,"Magic Attack")
